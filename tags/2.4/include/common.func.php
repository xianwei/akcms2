...
function sqlite_addslashes($string) {
...
function ak_htmlspecialchars($array) {//严重的逻辑混乱，命名也混乱
...
function parse_se($url) {//解析搜索引擎信息
	global $header_charset;
	$queries = querytoarray($url);
	if(in_string($url, array('www.baidu.com', 'www.baidu.cn', 'cache.baidu.com')) == 1) {
		$se_charset = 'GBK';
		$keyword = urldecode(getarrayvalue($queries, array('word', 'wd')));
		if(!empty($queries['ie']) && in_string($queries['ie'], array('UTF-8', 'utf-8'))) {
			$se_charset = 'UTF-8';
		}
		if($se_charset != $header_charset) {
			$keyword = @convcharset($se_charset, $header_charset, $keyword);
		}
		return Array('baidu', $keyword);
	} elseif(in_string($url, array('www.google.com', 'www.google.cn', 'www.google.de', 'google.co.jp')) == 1) {
		$se_charset = 'UTF-8';
		$keyword = urldecode(getarrayvalue($queries, array('q')));
		if($se_charset != $header_charset) {
			$keyword = @convcharset($se_charset, $header_charset, $keyword);
		}
		return Array('google', $keyword);
	} elseif(in_string($url, array('search.cn.yahoo.com', 'business.cn.yahoo.com/bso', 'coop.yahoo.com.cn/cnnic_right.html')) == 1) {
		$se_charset = 'GBK';
		$keyword = urldecode(getarrayvalue($queries, array('keyword', 'p')));
		if(!empty($queries['ei']) && in_string($queries['ei'], array('UTF-8', 'utf-8'))) {
			$se_charset = 'UTF-8';
		}
		if($se_charset != $header_charset) {
			$keyword = @convcharset($se_charset, $header_charset, $keyword);
		}
		return Array('yahoo', $keyword);
	} elseif(in_string($url, array('www.soso.com')) == 1) {
		$se_charset = 'GBK';
		$keyword = urldecode(getarrayvalue($queries, array('w')));
		if($se_charset != $header_charset) {
			$keyword = @convcharset($se_charset, $header_charset, $keyword);
		}
		return Array('soso', $keyword);
	} elseif(in_string($url, array('www.sogou.com')) == 1) {
		$se_charset = 'GBK';
		$keyword = urldecode(getarrayvalue($queries, array('query')));
		if($se_charset != $header_charset) {
			$keyword = @convcharset($se_charset, $header_charset, $keyword);
		}
		return Array('sogou', $keyword);
	} elseif(in_string($url, array('seek.3721.com')) == 1) {
		$parsed = parse_url($url);
		$array_query = explode('&', $parsed['query']);
		$keyword = '';
		foreach($array_query as $query) {
			list($key, $value) = explode('=', $query);
			if(in_array($key, Array('p', 'name'))) {
				$keyword = urldecode($value);
				break;
			}
		}
		return Array('3721', $keyword);
	} elseif(in_string($url, array('search.msn.com.cn')) == 1) {
		$se_charset = 'UTF-8';
		$keyword = urldecode(getarrayvalue($queries, array('q')));
		if($se_charset != $header_charset) {
			$keyword = @convcharset($se_charset, $header_charset, $keyword);
		}
		return Array('msn', $keyword);
	} elseif(in_string($url, array('zhongsou.com')) == 1) {
		$parsed = parse_url($url);
		$array_query = explode('&', $parsed['query']);
		$keyword = '';
		foreach($array_query as $query) {
			list($key, $value) = explode('=', $query);
			if(in_array($key, Array('w'))) {
				$keyword = urldecode($value);
				break;
			}
		}
		return Array('zhongsou', $keyword);
	} elseif(in_string($url, array('search.tom.com')) == 1) {
		$parsed = parse_url($url);
		$array_query = explode('&', $parsed['query']);
		$keyword = '';
		foreach($array_query as $query) {
			list($key, $value) = explode('=', $query);
			if(in_array($key, Array('word'))) {
				$keyword = urldecode($value);
				break;
			}
		}
		return Array('tom', $keyword);
	}
	return Array('', '');
}
