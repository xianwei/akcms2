...
function get_select_templates($type = '') {
	includecache('templates');
	global $templates;
	$selecttemplates = '';
	foreach($templates as $template) {
		if(strpos($template, $type.'_') === 0 || strpos($template, '_') === false) $selecttemplates .= "<option value=\"$template\">".$template."</option>\n";
	}
...
function get_item_data($id, $html = 0) {
	...
	includecache('itemexts');
	if(!empty($itemexts) && !empty($item['ext'])) {
		$itemextvalue = $db->get_one("SELECT * FROM {$tablepre}_item_exts WHERE id='{$id}'");
		foreach($itemexts as $ext) {
			$variables[$ext['Field']] = $itemextvalue[$ext['Field']];
		}
	}
...
//获得文件存放地址
	...
	$storemethod = get_category_storemethod($category);
...
function aexit($message = '') {
	global $db, $debug, $sysname, $sysedition, $mtime;
	$db = db();
	$str_debug = $message;
	...
	if(isset($db)) {
		if(empty($debug)) {
			$str_debug .= "<center><div style=\"width: 100px;margin-top: 10px;\" class=\"mininum\"><img src=\"".AK_URL."images/admin/query.gif\" style=\"float: left\">".$db->querynum.'&nbsp;&nbsp;Time:'.$exetime.'</div>';
		} else {
			$str_debug .= "<center><div style=\"width: 100px;margin-top: 10px;\" class=\"mininum\"><img src=\"".AK_URL."images/admin/query.gif\" style=\"float: left\" onclick=\"show_query_debug()\" style=\"cursor: hand\">".$db->querynum.'&nbsp;&nbsp;Time:'.$exetime.'</div>';
...
function operatespiderlist($spiderid, $write = 1) {
...
function spiderurl($ruleid, $url, $linktext = '') {//根据采集规则和URL得到一个结果数组
	...
	foreach($itemexts as $ext) {
		$f = $ext['Field'];
		if($rule[$f] != '') $extflag = 1;
		$$f = ak_replace($array_replace, $array_to, $rule[$f]);
		$return[$f] = $$f;
	}
...
function getstatby($start, $end, $type = 'day') {
	...
	$sql = "SELECT * FROM {$tablepre}_stats WHERE dateline>='$start' AND dateline<'$end' AND `by`='$type' AND type IN (0,1) ORDER BY dateline";
	...
	for($time = getwee($start, $type); $time < $end;) {
		...
		if(!isset($pv) || !isset($ip)) {
			...
			$ip = $stat['ip'];
			if($time < $wee) {
				$db->query("REPLACE INTO {$tablepre}_stats(dateline,type,value,finished,`by`)VALUES('$time','0','$pv',1,'$type'),('$time','1','$ip',1,'$type')");
			}
		}
...
function extfieldinput($field) {
	...
	if($type == 'string' || $type == 'number') {
		return "<input type=\"text\" name=\"{$field['alias']}\" value=\"{$field['standby']}\">";
	} elseif($type == 'select') {
		$return = '';
		$items = explode("\n", $standby);
		...
		return "<select name=\"{$field['alias']}\">{$return}</select>";
	} elseif($type == 'radio') {
		$return = '';
		$items = explode("\n", $standby);
		$i = 0;
		foreach($items as $item) {
			...
			if(isset($f[1])) $t = $f[1];
			$id = "{$field['alias']}_{$i}";
			$return .= "<input type=\"radio\" id=\"$id\" name=\"{$field['alias']}\" value=\"{$v}\"><label for=\"{$id}\">{$t}</label>\n";
		}
		return $return;
	}
}
