...
if($get_action == 'phpinfo') {
	...
	$sql = "OPTIMIZE TABLE `{$tablepre}_admins` , `{$tablepre}_attachments` , `{$tablepre}_categories` , `{$tablepre}_filenames` , `{$tablepre}_items` , `{$tablepre}_sections` , `{$tablepre}_settings` , `{$tablepre}_stats` , `{$tablepre}_texts` , `{$tablepre}_variables`";
	$db->query($sql);
	...
	if(isset($post_sql))  {
		...
		if(is_resource($query)) {
			...
			if($body != '') echo "<table class=\"commontable\" cellspacing=\"1\">{$header}{$body}</table>";
		}
	}
