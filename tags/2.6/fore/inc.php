...
function dealwithstatcache() {
	...
	if(filesize($statcachefilename) > $setting_statcachesize || $wee > $lastmodified) {
		...
		$db->query("REPLACE INTO {$tablepre}_stats(dateline,value,type,`bywhat`)VALUES('$wee','$visit_all', 0, 0)");
		$db->query("REPLACE INTO {$tablepre}_stats(dateline,value,type,`bywhat`)VALUES('$wee','$ip_all', 1, 0)");
