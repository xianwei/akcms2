...
if(!isset($get_action)) {
	...
	$categoriestree = rendercategorytree();
	$smarty->assign('categoriestree', $categoriestree);
	...
	if(isset($post_category_new_submit)) {
		...
		$smarty->assign('get_parent', $get_parent);
	...
	if(!isset($post_title)) {
		...
		if(isset($cookie_lastcategory)) $smarty->assign('lastcategory', $cookie_lastcategory);
		...
		setcookie('lastcategory', $post_category);
		...
		if($post_category == -1) $values['orderby'] = strlen($post_title);
		$db->insert('items', $values);
		$itemid = $db->insert_id();
		if($post_category == -1) updatecache('keywords');
		if(!empty($post_data)) {
			$value = array(
				'itemid' => $itemid,
				'text' => $post_data,
	...
	if(!isset($get_job) && !isset($get_id)) {
		...
		if(!isset($post_saveeditpage)) {
			...
			if($db->get_by('*', 'filenames', "id='$get_id'")) {
				...
				$db->update('filenames', $value, "id='$get_id'");
			} else {
				$value = array(
					'filename' => $htmlfilename,
					'id' => $get_id,
	...
	if(!isset($post_title)) {
		...
		$smarty->assign('maxattachsize', $setting_maxattachsize * 1024);
		$smarty->assign('usefilename', $setting_usefilename);
		$smarty->assign('itemdateline', date('Y-m-d H:i:s', $item['dateline']));
		if(isset($cookie_lastcategory)) $smarty->assign('lastcategory', $cookie_lastcategory);
		if(!empty($item['ext'])) {
		...
		$post_time = str_replace(':', '-', $post_time);
		list($y, $m, $d, $h, $i, $s) = explode('-', $post_time);
		...
		batchhtml(array($get_id));
		if($item['category'] == -1 || $post_category == -1) updatecache('keywords');
		adminmsg($lan['operatesuccess'], 'admincp.php?action=items');
	}
	...
	if(!empty($setting_forbidspider)) adminmsg($lan['functiondisabled'].'<br><br><a href="setting.php?action=functions">'.$lan['open'].'</a>', '', 3, 1);
	if(!isset($get_job)) {
		includecache('categories');
		includecache('sections');
		...
		while($spider = $db->fetch_array($query)) {
			...
			<td><a href=\"admincp.php?action=spiders&job=exportspider&id={$spider['id']}\" target=\"_blank\">{$lan['export']}</a></td>
		...
		while($row = $db->fetch_array($query)) {
			$rule = ak_unserialize($row['data']);
			$spiderruleslist .= "<tr><td>{$row['id']}</td>";
			$spiderruleslist .= "<td><a href=\"admincp.php?action=spiders&job=editrule&id={$row['id']}\">{$rule['spiderrulename']}</a></td>";
			$spiderruleslist .= "<td><a href=\"admincp.php?action=spiders&job=previewrule&id={$row['id']}\" target=\"_blank\">{$lan['preview']}</a></td>";
			$spiderruleslist .= "<td><a href=\"admincp.php?action=spiders&job=exportspiderrule&id={$row['id']}\" target=\"_blank\">{$lan['export']}</a></td>";
			$spiderruleslist .= "<td><a href=\"javascript:deleterule({$row['id']})\">".alert($lan['delete'])."</a></td></tr>";
		}
		...
		if(!isset($post_submitflag)) {
			...
			$smarty->assign('selecttemplates', $selecttemplates);
			$smarty->assign('selectrules', $selectrules);
			displaytemplate('admincp_spider_new.htm');
		} else {
		...
		if(!isset($post_submitflag)) {
			...
			displaytemplate('admincp_spiderrule_new.htm');
		} else {
			...
			//ext end
			$db->insert('spiderrules', array('data' => serialize($value)));
			$ruleid = $db->insert_id();
		...
		$data = $db->get_by('data', 'spiderrules', "id='$get_id'");
		$rule = ak_unserialize($data);
		...
		$keys = $lan;
		$extfields = ak_unserialize($settings['extfields']);
		foreach($extfields as $key => $extfield) {
			$keys[$key] = $extfield['name'];
		}
		...
		//ext end
		$db->update('spiderrules', array('data' => serialize($value)), "id='$get_id'");
		updatecache('spiderrules');
	...
	} elseif($get_job == 'exportspiderrule') {
		if(empty($get_id) || !a_is_int($get_id)) adminmsg($lan['parameterwrong']);
		$rule = $db->get_by('*', 'spiderrules', "id='$get_id'");
		exit("<textarea cols='100' rows='10'>".base64_encode(serialize($rule))."</textarea>");
	} elseif($get_job == 'importspiderrule') {
		if(empty($post_data)) {
			displaytemplate('admincp_importspiderrule.htm');
		} else {
			$value = unserialize(base64_decode($post_data));
			unset($value['id']);
			$db->insert('spiderrules', $value);
			adminmsg($lan['operatesuccess'], 'admincp.php?action=spiders');
		}
	} elseif($get_job == 'exportspider') {
		if(empty($get_id) || !a_is_int($get_id)) adminmsg($lan['parameterwrong']);
		$spider = $db->get_by('*', 'spiders', "id='$get_id'");
		exit("<textarea cols='100' rows='10'>".base64_encode(serialize($spider))."</textarea>");
	} elseif($get_job == 'importspider') {
		if(empty($post_data)) {
			displaytemplate('admincp_importspider.htm');
		} else {
			$value = unserialize(base64_decode($post_data));
			unset($value['id']);
			$db->insert('spiders', $value);
	...
	} elseif($get_job == 'source') {
		debug(readfromurl($get_url));
	}
	...
	if(!empty($get_job) && $get_job == 'delete') {
		if(isset($get_key)) {
			unset($extfields[$get_key]);
		}
	...
	if(!empty($get_job) && $get_job == 'newext') {
		if($post_alias == '') adminmsg('error', 'admincp.php?action=itemexts', 3, 1);
	...
	if($extfields) {
		foreach($extfields as $key => $extfield) {
			$exts .= "<tr><td>{$extfield['name']}</td><td>{$key}</td><td>{$lan[$extfield['type']]}</td><td>".extfieldinput($extfield)."</td><td><a href='admincp.php?action=itemexts&job=delete&key=".$key."' onclick='return confirmdelete()'>{$lan['delete']}</a></td></tr>";
		}
	}
	$smarty->assign('exts', $exts);
	...
	debug('manual');
} elseif($get_action == 'crons') {
	if(empty($get_job)) {
		$str_crons = '';
		$query = $db->list_by('*', 'crons');
		while($cron = $db->fetch_array($query)) {
			$str_crons .= "<tr><td>{$cron['id']}</td><td>{$cron['name']}</td><td>{$cron['data']}</td></tr>";
		}
		$smarty->assign('str_crons', $str_crons);
		displaytemplate('admincp_crons.htm');
	} elseif($get_job == 'new') {
		if(empty($post_name) || empty($post_url) || empty($post_type)) adminmsg($lan['cronerror'], 'back', 3, 1);
		$value = array(
			'name' => $post_name,
			'data' => $post_url,
			'job' => 'url',
